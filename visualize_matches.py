...
COLOR_WARNING = (255, 0, 255) # Bright Magenta (RGB)

# Global counter for missing text entries
missing_counter = 0
...
def draw_chapter_recursive(draw_obj, font, chapter, x_offset, args, level=0):
    ...
    for field in fields:
        ...
        # Determine if text is missing [cite: 2026-02-26]
        is_missing = field_text is None or (isinstance(field_text, str) and not field_text.strip())

        if bbox and len(bbox) == 2:
            if is_missing:
                missing_counter += 1 # Update console report counter [cite: 2026-02-26]

            # Define Left and Right coordinates correctly [cite: 2026-02-26]
            ...
            # Draw Bounding Boxes [cite: 2026-02-26]
            if args.p_bbox:
            ...
            if args.p_text:
                draw_obj.text((p1_l[0] + 3, p1_l[1] + 2), txt, font=font, fill=text_color)
            if args.e_text:
                draw_obj.text((p1_r[0] + 3, p1_r[1] + 2), txt, font=font, fill=text_color)

    # Draw Lines [cite: 2026-02-26]
